...
struct Line {
  ...
  Point center() {
    return (this->vertices[0] + this->vertices[1]) / 2.;
  }
};
...
std::string to_string(const Line &line) {
  return fmt::format("{}, {}", to_string(line.vertices[0]), to_string(line.vertices[1]));
}
...
std::string to_string(const Circle &circle) {
  return fmt::format("center:{}, radius:{}", to_string(circle.center), circle.radius);
}
