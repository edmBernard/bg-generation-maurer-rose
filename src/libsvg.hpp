//
//  https://github.com/edmBernard/bg-generation-maurer-rose
//
//  Created by Erwan BERNARD on 01/05/2024.
//
//  Copyright (c) 2024 Erwan BERNARD. All rights reserved.
...
struct Fill : public Color {
  ...
  Fill(uint32_t hexColor, float opacity = 1)
...
std::string to_draw(const Line &tr) {
  return fmt::format("M {} {} L {} {}", tr.vertices[0].x, tr.vertices[0].y, tr.vertices[1].x, tr.vertices[1].y);
}
...
std::string to_style(std::optional<Fill> fill) {
  return fill ? fmt::format("fill:rgb({},{},{});fill-opacity:{}", fill->r, fill->g, fill->b, fill->opacity) : "fill:none";
}
...
public:
  ...
  [[nodiscard]] bool save(std::filesystem::path filename) {
    ...
    out << "<svg xmlns='http://www.w3.org/2000/svg' "
        << fmt::format("height='{height}' width='{width}' viewBox='0 0 {width} {height}'>\n", fmt::arg("height", canvasHeight), fmt::arg("width", canvasWidth))
  ...
  void addCircle(const Circle &circle, std::optional<Fill> fill, std::optional<Stroke> stroke) {
    content += fmt::format("<circle style='{};{}' cx='{}' cy='{}' r='{}' />\n", details::to_style(fill), details::to_style(stroke), circle.center.x, circle.center.y, circle.radius);
  }
  ...
  void addPath(const T &shapes, std::optional<Fill> fill, std::optional<Stroke> stroke, Lambda func = [](const typename T::value_type &) { return true; }) {
    ...
    content += fmt::format("<path style='{};{};stroke-linecap:round' d='{}'></path>\n", details::to_style(fill), details::to_style(stroke), s_path);
  }

  void addLine(const Line &shape, std::optional<Fill> fill, std::optional<Stroke> stroke) {
    content += fmt::format("<path style='{};{};stroke-linecap:round' d='{}'></path>\n", details::to_style(fill), details::to_style(stroke), details::to_draw(shape));
  }
